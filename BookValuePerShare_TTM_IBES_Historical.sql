/*
    ...
    @DESC   :   THIS QUERY PULL HISTORICAL TTM BOOK VALUE PER SHARE.

     
    @EDITS  :   4/18/2018: UPDATED QUERY LOGIC TO REMOVE ISNULL FUNCTION. NULL VALUES WILL NOW CAUSE FINAL VALUE TO RESULT IN NULL. UPDATED QUERY LOGIC TO PESUDO NULL VALUES IN THE ACTUAL REPORTED AND ESTIMATE IBES TABLES. -99999 VALUES NOW SHOW AS NULL
    @EDITS  :   5/14/2018: ADDED BPN TO GBP CURRENCY CONVERSION AND NAMING FOR USESTDATA CTE. 
                ...
                7/03/2018: CHANGED JOIN TO LEFT JOIN FOR E2 TABLES
...
, USACTDATA AS (SELECT A.CODE, A.DATE_, A.FYDATE, (NULLIF(A.FYVALUE,-99999)/ISNULL(B.SPLITFACTOR,1))  AS FYVALUE -- CASE STATEMENT TO CONVERT PENCE TO POUNDS 
                ...
                WHERE  A.DATE_ >= @STARTDATE AND A.DATE_ < @ENDDATE  AND A.MEASURE=1)
...
, GACTDATA AS (SELECT A.CODE, A.DATE_, A.FYDATE, (NULLIF(A.FYVALUE,-99999)/ISNULL(B.SPLITFACTOR,1))  AS FYVALUE -- CASE STATEMENT TO CONVERT PENCE TO POUNDS 
               FROM (SELECT *, '1' AS HISTORICALBASISFLAG FROM IBGSACTL3 UNION SELECT *, '2' AS HISTORICALBASISFLAG FROM IBGS2NDACTL3) A
                ...
                WHERE A.DATE_ >= @STARTDATE AND A.DATE_ < @ENDDATE AND A.MEASURE=1 AND A.HISTORICALBASISFLAG=A1.HISTORICALBASISFLAG)
...
, USESTDATA AS (SELECT A.CODE,A.MEASURE,A.ESTDATE, A.PERDATE,A.PERIOD, A.PERTYPE,CASE WHEN C.CURRENCY_= 'BPN' THEN 'GBP' ELSE C.CURRENCY_ END AS QAD_CURRENCY_ --,D.CURRENCY_ AS DFO_CURRENCY_
                ...
                WHERE A.ESTDATE >= @STARTDATE AND A.ESTDATE < @ENDDATE  AND A.MEASURE=1 AND A.PERTYPE=1)
    
---- IBES SUMMARY GLOBAL ESTIMATE DATA TO GET FY1, FY2 & FY3 MEAN DATA
, GESTDATA AS (SELECT A.CODE,A.MEASURE,A.ESTDATE, A.PERDATE,A.PERIOD, A.PERTYPE,CASE WHEN C.CURRENCY_= 'BPN' THEN 'GBP' ELSE C.CURRENCY_ END AS QAD_CURRENCY_ --,D.CURRENCY_ AS DFO_CURRENCY_
                    , CASE WHEN C.CURRENCY_= 'BPN' THEN (NULLIF(A.MEAN,-99999)/ISNULL(B.SPLITFACTOR,1))/100 ELSE (NULLIF(A.MEAN,-99999)/ISNULL(B.SPLITFACTOR,1)) END AS MEAN -- CASE STATEMENT TO CONVERT PENCE TO POUNDS 
                ...
                WHERE A.ESTDATE >= @STARTDATE AND A.ESTDATE < @ENDDATE  AND A.MEASURE=1 AND A.PERTYPE=1 AND A.HISTORICALBASISFLAG=A1.HISTORICALBASISFLAG)
...
-- ISNULL(
--      CASE WHEN F.QAD_CURRENCY_ = F.DFO_CURRENCY_ THEN ROUND(F.BPS_TTM,2)
--           WHEN F.QAD_CURRENCY_ = 'GBP' AND F.DFO_CURRENCY_ = 'BPN' THEN ROUND(F.BPS_TTM*100,2)
--           ELSE ROUND(F.BPS_TTM*(TGTFX.MIDRATE/GBPFX.MIDRATE),2) END,
--      CASE WHEN F.QAD_CURRENCY_ = F.DFO_CURRENCY_ THEN ROUND(F.FY1_BPS,2)
--           WHEN F.QAD_CURRENCY_ = 'GBP' AND F.DFO_CURRENCY_ = 'BPN' THEN ROUND(F.FY1_BPS*100,2)
--           ELSE ROUND(F.FY1_BPS*(TGTFX.MIDRATE/GBPFX.MIDRATE),2) END
--      ) AS DFO_BPS_TTM
...
WHERE F.BPS_TTM IS NOT NULL
-- AND F.ESTDATE = (SELECT MAX(ESTDATE) FROM T_12 X WHERE X.ID = F.ID AND X.ESTDATE <= GETDATE()) -- TO REPLICATE DFO 'STATIC REQUEST' FUNCTIONALITY